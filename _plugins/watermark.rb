  ...
  class WatermarkGenerator < Generator
    ...
    def generate(site)
      Dir.glob('assets/images/**/*').each do |file|
        watermarked_path = destination_path(file, "watermarked")
        if (File.file?(file))
          watermark_image(site, file, watermarked_path)
          create_thumb(site, watermarked_path)
	  add_to_static_file(site, file)
        end
      end
    end
    ...
    def destination_path(file, directory)
      new_file = String.new(file)
      new_file["assets/images"] = directory
    ...
    def watermark_image(site, file, new_file)
      ...
      result = image.composite(MiniMagick::Image.open('assets/watermark-large.png')) do |c|
        c.gravity "southeast"
      end
    ...
    def create_thumb(site, file)
      ...
      result = image.composite(MiniMagick::Image.open('assets/watermark-small.png')) do |c|
        c.gravity "southeast"
      end
